...
def to_frame(src, dst, dat, allow_share=True):
    ...
    src_port = src[1]
    dst_port = dst[1]
    assert dst_addr[0] == 0x00 or dst_addr[0] == 0x08
    ...
    if src_port == CDNET_DEF_PORT:  # out request, backup dst port if need at outside
        ...
        if allow_share and len(dat) >= 1 and dat[0] <= 31:
            # share first byte
            payload = bytes([dat[0] | HDR_L0_REPLY | HDR_L0_SHARE]) + dat[1:]
        else:
...
def from_frame(frame, local_net=0, last_port=None):
    ...
    if hdr & HDR_L0_REPLY:  # in reply
        ...
        dst_port = CDNET_DEF_PORT
        if hdr & HDR_L0_SHARE:
            dat = bytes([remains[0] & 0x1f]) + remains[1:]
        else:
            dat = remains[1:]
    else:                   # in request
